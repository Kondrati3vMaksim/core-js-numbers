...
function getAngleBetweenVectors(x1, y1, x2, y2) {
  const lengthVec = x1 * x2;
  const lengthSec = y1 * y2;
  const sum = lengthVec + lengthSec;
  const length1 = Math.sqrt(x1 ** 2 + y1 ** 2);
  const length2 = Math.sqrt(x2 ** 2 + y2 ** 2);
  const cos = sum / (length1 * length2);
...
function getSumOfDigits(num) {
  ...
  const toStringArray = toString.split('');
  toStringArray.forEach((mass) => {
    const nume = Number(mass);
    a += nume;
  });
  return a;
}
